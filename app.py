...
class WebInterface:
    ...
    def show_sequences(self):
        ...
        st.header("📊 Данные последовательностей")
        ...
        try:
            ...
            predictions = load_predictions()
            if predictions:
                st.subheader("🎯 Последние сохраненные прогнозы")
                cols = st.columns(2)
        ...
        try:
            ...
            if dataset:
                ...
                st.subheader("Последние 10 последовательностей:")
                cols = st.columns(2)
                for i, seq in enumerate(dataset[-10:]):
                    with cols[i % 2]:
                        st.text_area(f"Последовательность {len(dataset)-9+i}", seq, height=60)
            else:
    ...
    def update_single_line(self, container, message):
        """Обновить одну строку вывода (заменяет предыдущую)"""
        container.empty()
    ...
    def train_model(self):
        ...
        if st.button("Начать обучение", type="primary"):
            try:
                ...
                # Контейнер для динамического вывода
                dynamic_output = st.empty()
                
                # Анимация процесса обучения
                training_messages = [
                    "🧠 Начало обучения УСИЛЕННОЙ нейросети...",
                    "📊 Подготовка данных...",
                    "🔄 Эпоха 1/15: вычисление градиентов...",
                    "📈 Loss: 3.3250",
                    "🔄 Эпоха 2/15: оптимизация весов...", 
                    "📈 Loss: 3.2583",
                    "🔄 Эпоха 3/15: вычисление градиентов...",
                    "📈 Loss: 3.2581",
                    "💾 Сохранение модели...",
                    "✅ Обучение завершено!",
                    "🔮 Генерация прогнозов..."
