# [file name]: ml/ensemble/ensemble.py
"""
Ансамблевые предсказания с комбинированием моделей - РЕФАКТОРИНГ ДЛЯ НОВОЙ СТРУКТУРЫ
"""
...
from ml.features.advanced import AdvancedPatternAnalyzer, FrequencyBasedPredictor, SmartNumberSelector
...
class StatisticalPredictor:
    ...
    def predict(self, history: List[int], top_k: int = 10) -> List[Tuple[Tuple[int, int, int, int], float]]:
        ...
        candidates = self._generate_statistical_candidates(limited_history, patterns, top_k)
...
class EnsemblePredictor:
    """Основной ансамблевый предсказатель"""
    ...
    def _get_predictor(self, name: str):
    ...
    def _safe_predict(self, predictor, history: List[int], top_k: int):
...
def create_ensemble_predictor():
    """Создание ансамблевого предсказателя (для обратной совместимости)"""
